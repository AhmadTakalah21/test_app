...
import 'package:appointments_app/global/widgets/main_snack_bar.dart';
  ...
  Future<void> onCreateWorkspace() async {
    ...
    try {
      ...
      if (authCubit.state is! TenantAvailabilitySuccess) {
        MainSnackBar.showErrorMessage(context, 'something_went_wrong'.tr());
        return;
      }
      ...
      if (availability.state == 1) {
        // الاسم محجوز
        MainSnackBar.showErrorMessage(context, 'tenant_unavailable'.tr());
        return;
      }
      ...
      if (needEditionId) {
        ...
        if (authCubit.state is! EditionsForSelectSuccess) {
          ...
          MainSnackBar.showErrorMessage(context, err);
          return;
        }

        final model = (authCubit.state as EditionsForSelectSuccess).model;
      ...
      //MainSnackBar.showErrorMessage(context, friendly);
    } finally {
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: Column(
                          ...
                          children: const [AuthTail(), SizedBox(height: 20)],
                        ),
                      ),
                    ],
                  ),
  ...
  Widget _buildField({
    ...
    return MainTextField(
      ...
      prefixIcon: Padding(
        ...
        child: Icon(icon, size: 16, color: AppColors.blackShade),
      ),
      showCloseIcon: true,
    ...
    return BlocConsumer<AuthCubit, AuthState>(
      listener: (context, state) {
        if (state is RegisterSuccess) {
          ...
          authCubit
            ..setSignInTenant(model.tenancyName)
            ..setSignInEmailOrUser(model.emailAddress)
            ..setSignInPassword(authCubit.registerModel.adminPassword)
